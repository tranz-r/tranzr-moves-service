namespace TranzrMoves.Domain.Constants;

public static class FromEmails
{
    public const string Booking = "bookings@tranzrmoves.com";
    public const string Support = "support@tranzrmoves.com";
    public const string NoReply = "no-reply@tranzrmoves.com";
    public const string Sales = "sales@tranzrmoves.com";
    public const string Group = "tranzrgroup@tranzrmoves.com";
}

public static class ToBccEmails
{
    public const string PayLaterWarning = "payment-warning@tranzrmoves.com";
}

