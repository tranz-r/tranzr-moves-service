...
public class QuoteV2PaymentSheetIntegrationTests(TestServerFixture fixture)
    : IClassFixture<TestServerFixture>, IAsyncLifetime
{
    ...
    [Fact]
    public async Task PostPaymentSheet_Returns400_When_Request_Is_Invalid()
    {
        var request = new CreateQuoteV2PaymentSheetRequest
        {
            QuoteId = Guid.Empty,
            ExpectedVersion = 0,
    ...
    [Fact]
    public async Task PostPaymentSheet_ReturnsNotFound_When_Quote_Does_Not_Exist()
    {
        var request = new CreateQuoteV2PaymentSheetRequest
        {
    ...
    [Fact]
    public async Task PostPaymentSheet_Returns412_When_ExpectedVersion_Does_Not_Match()
    {
        ...
        {
            Id = userId,
            Email = "sheet-v2@tranzrmoves.com",
            FirstName = "S",
            LastName = "V2",
            CreatedAt = now,
            ModifiedAt = now,
        ...
        {
            Id = quoteId,
            SessionId = $"psv2-{quoteId:N}"[..14],
            Type = QuoteType.Removals,
            QuoteReference = $"PSV2-{quoteId:N}"[..16],
